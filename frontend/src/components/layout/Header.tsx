...
export const Header: React.FC<HeaderProps> = ({ setIsMobileOpen }) => {
  ...
  return (
    <header 
      className={`h-16 bg-white/80 backdrop-blur-md border-b border-slate-200 sticky top-0 z-30 transition-all duration-200 flex items-center justify-between px-4 sm:px-6 lg:px-8
        ${scrolled ? 'shadow-sm' : ''}
      `}
    >
      ...
      <div className="flex items-center gap-3 sm:gap-4">
        ...
        <div className="hidden sm:flex items-center relative group">
          ...
          <input 
            type="text" 
            placeholder="Buscar..." 
            className="pl-9 pr-4 py-1.5 bg-slate-100 border-transparent rounded-full text-sm focus:bg-white focus:border-primary-300 focus:ring-2 focus:ring-primary-100 w-48 lg:w-64 transition-all duration-300 outline-none placeholder:text-slate-500"
          />
        </div>

        <button className="relative p-2 rounded-full text-slate-500 hover:bg-slate-100 transition-colors">
          <Bell size={20} />
          <span className="absolute top-1.5 right-1.5 w-2 h-2 bg-danger-500 rounded-full border border-white pulse-dot"></span>
        </button>
