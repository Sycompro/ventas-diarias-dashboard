...
export function DataTable<T>({ title, columns, data, isLoading, emptyMessage = 'No hay datos disponibles' }: DataTableProps<T>) {
  ...
  return (
    <div className="bg-white rounded-xl shadow-sm  -slate-200 overflow-hidden flex flex-col">
      <div className="px-5 py-4  -slate-100 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div className="relative">
          ...
          <input
            type="text"
            placeholder="Buscar..."
            className="pl-9 pr-4 py-2 text-sm  -slate-200 rounded-lg focus:outline-none focus:-blue-500 focus:ring-1 focus:ring-blue-500 transition-shadow w-full sm:w-64"
            value={searchTerm}
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-left -collapse">
          <thead className="bg-slate-100/50">
            <tr className="bg-slate-50/80  -slate-200">
              {columns.map((col, idx) => (
      ...
      <div className="px-5 py-3.5  -slate-200 bg-slate-50/50 flex items-center justify-between">
        <span className="text-xs text-slate-500">Mostrando {data.length} registros</span>
        <div className="flex gap-1.5">
          <button className="px-3 py-1.5 text-xs font-medium text-slate-600 bg-white  -slate-200 rounded-md hover:bg-slate-50 disabled:opacity-50 transition-colors shadow-sm">
            Anterior
          </button>
          <button className="px-3 py-1.5 text-xs font-medium text-slate-600 bg-white  -slate-200 rounded-md hover:bg-slate-50 disabled:opacity-50 transition-colors shadow-sm">
            Siguiente
