...
export const SalesPage: React.FC = () => {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Layout de dos columnas: Tabla Pivot + Ventas por Categoría */}
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 items-start animate-in fade-in duration-700 delay-200">
        
        {/* Cuadro Estadístico Pivot (2/3 de ancho) */}
        ...
        {/* Ventas por Categoría: Productos vs Servicios (1/3 de ancho) */}
        <div className="lg:col-span-1 bg-white rounded-xl border border-slate-200/80 overflow-hidden flex flex-col">
          ...
          <div className="p-5 flex-1 flex flex-col justify-between space-y-6">
            {loadingPivot ? (
              ...
              <>
                ...
                <div className="space-y-6">
                  ...
                  <div className="space-y-2">
                    <div className="flex justify-between items-center text-xs">
                      ...
                      <div className="text-right">
                        <span className="font-extrabold text-slate-900 tabular-nums">{formatCurrency(metrics?.byItemType?.products || 0)}</span>
                        <span className="text-slate-400 font-semibold ml-1.5 text-[11px] tabular-nums">
                  ...
                  <div className="space-y-2">
                    <div className="flex justify-between items-center text-xs">
                      ...
                      <div className="text-right">
                        <span className="font-extrabold text-slate-900 tabular-nums">{formatCurrency(metrics?.byItemType?.services || 0)}</span>
                        <span className="text-slate-400 font-semibold ml-1.5 text-[11px] tabular-nums">
                ...
                {/* Resumen Informativo en Caja Sutil */}
                <div className="p-3 bg-slate-50/50 border border-slate-200/50 rounded-xl text-center">
                  <p className="text-[10px] font-bold text-slate-500 leading-relaxed uppercase">
                    {(metrics?.byItemType?.products || 0) > (metrics?.byItemType?.services || 0) ? (
                      "Predominan ventas de bienes físicos"
                    ) : (metrics?.byItemType?.services || 0) > (metrics?.byItemType?.products || 0) ? (
                      "Predominan ventas de servicios"
                    ) : (
